...
library ConfiguratorLib {
  ...
  function updateConfigs(
    ...
    if (!isValidUpdate(reservePools_, rewardPools_, triggerData_, configUpdates_, manager_)) {
  ...
  function applyConfigUpdates(
    ...
    for (uint256 i = numExistingRewardPools_; i < configUpdates_.rewardPoolConfigs.length; i++) {
      initializeRewardPool(rewardPools_, receiptTokenFactory_, configUpdates_.rewardPoolConfigs[i]);
    }
